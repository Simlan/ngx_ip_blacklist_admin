...
!function (e) {
    ...
    h.prototype = {
        constructor: h, init: function (e) {
            ...
            var i = this, t = "", n = i.initialPreview, a = i.initialPreviewCount, r = i.initialCaption.length, l = "preview-" + g(), o = r > 0 ? i.initialCaption : i.msgSelected.replace(/\{n\}/g, a), s = e(o).text();
            if (c(n) && a > 0) {
            ...
            var n = this, a = f.createObjectURL(i), r = e("#" + t), l = v("other", n.previewTemplates) ? n.previewTemplates.other : o.other;
            ...
            var n, a, r = this, l = r.parseFileType(e), p = r.slug(e.name), c = r.allowedPreviewTypes, m = r.allowedPreviewMimeTypes, u = (e.type, v(l, r.previewTemplates) ? r.previewTemplates[l] : o[l]), h = v(l, r.previewSettings) ? r.previewSettings[l] : s[l], b = parseInt(r.wrapTextLength), y = r.wrapIndicator, C = r.$preview, $ = c.indexOf(l) >= 0, x = d(m) || !d(m) && v(e.type, m);
            if ($ && x) {
                if ("text" == l) {
                    var F = w(i.target.result);
                    if (f.revokeObjectURL(n), F.length > b) {
                        var T = "text-" + g(), I = .75 * window.innerHeight, P = r.getLayoutTemplate("modal").replace(/\{id\}/g, T).replace(/\{title\}/g, p).replace(/\{height\}/g, I).replace(/\{body\}/g, F);
            ...
            function i(p) {
                ...
                var v, g, y, C, $, x = u + "-" + p, F = e[p], T = t.slug(F.name), I = (F.size ? F.size : 0) / 1e3, P = f.createObjectURL(F), E = 0, L = t.allowedFileTypes, S = d(L) ? "" : L.join(", "), R = t.allowedFileExtensions, j = d(R) ? "" : R.join(", "), k = d(R) ? "" : new RegExp("\\.(" + R.join("|") + ")$", "i");
            ...
            var t = this, n = t.$element, a = t.$preview, r = t.reader, l = t.$previewContainer, o = t.$previewStatus, s = t.msgLoading, m = t.msgProgress, u = (t.msgSelected, t.previewFileType, parseInt(t.wrapTextLength), t.wrapIndicator, "preview-" + g()), w = e.length, h = t.fileTypeSettings, b = v("text", h) ? h.text : p.text;
            i(0)
        }, slug: function (e) {
            return d(e) ? "" : e.split(/(\\|\/)/g).pop().replace(/[^\w-.\\\/ ]+/g, "")
        }, change: function (i) {
            var t, n = this, a = n.$element, r = n.slug(a.val()), l = 0, o = n.$preview, s = a.get(0).files, p = n.msgSelected, c = d(s) ? 1 : s.length + n.initialPreviewCount;
            if (n.hideFileIcon(), t = void 0 === i.target.files ? i.target && i.target.value ? [{name: i.target.value.replace(/^.+\\/, "")}] : [] : i.target.files, 0 !== t.length) {
                ...
                n.isError ? (n.$captionContainer.find(".kv-caption-icon").hide(), m = n.msgValidationError) : n.showFileIcon(), n.$caption.html(m), n.$captionContainer.attr("title", e(m).text()), n.$container.removeClass("file-input-new"), a.trigger("fileselect", [c, r])
            }
